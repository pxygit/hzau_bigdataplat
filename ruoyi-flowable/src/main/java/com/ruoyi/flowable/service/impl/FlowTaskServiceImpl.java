...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public AjaxResult complete(FlowTaskVo taskVo) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            ...
            if (CollectionUtils.isEmpty(nextUserTask)) {
                String applyFormType = null;
                for (String key : variables.keySet()) {
                    if (key.startsWith("applyFormType")) {
                        String[] parts = key.split("_");
                        String number = parts[parts.length - 1];
                        applyFormType = (String) variables.get(key);
                        if(!applyFormType.equals(number)){
                            return AjaxResult.error(key +"表单类型不匹配");
                        }
                        break;  // 找到第一个匹配的值
                    }
                }
                if (applyFormType.equals("1")) {
                    //如果字段值为"1",则调用completeAccountApply
                    completeAccountApply(taskVo.getVariables());
    ...
    @Transactional(rollbackFor = Exception.class)
    public void completeAccountApply(Map<String, Object> variables) {
        // 获取表单变量值
        String username = (String) variables.get("username1");
        String nickname = (String) variables.get("nickname1");
        String phone = (String) variables.get("phone1");
        String sex = (String) variables.get("sex1");
        String email = (String) variables.get("email1");

        // 创建de_sys_user对象并插入
        DeSysUser deSysUser = new DeSysUser();
        deSysUser.setUsername(username);
        deSysUser.setNickName(nickname);
        deSysUser.setPhone(phone);
        deSysUser.setGender(Objects.equals(sex, "0") ? "男" : "女"); // 统一性别, 0 男, 1 女
        deSysUser.setEmail(email);
        deSysUser.setPassword(MD5Util.computeMD5(username+nickname+phone+email+"hzau"));
        deSysUser.setIsAdmin(false);
        deSysUser.setEnabled(1L);
        deSysUser.setCreateBy("system");
        long now = System.currentTimeMillis();
        deSysUser.setCreateTime(now);
        deSysUser.setUpdateTime(now);
        deSysUser.setFrom(0);
        deSysUser.setLanguage("zh_CN");
        deSysUser.setPwdResetTime(now);
        deSysUserMapper.insertDeSysUser(deSysUser);
        // 还需要插入用户角色表
        DeSysUsersRoles deSysUsersRoles = new DeSysUsersRoles();
        deSysUsersRoles.setUserId(deSysUser.getUserId());
        deSysUsersRoles.setRoleId(2L);
        deSysUsersRolesMapper.insertDeSysUsersRoles(deSysUsersRoles);

        // 查询sys_user表获取user_id
        SysUser sysUser = sysUserMapper.selectUserByUserName(username);
        if (sysUser == null) {
            throw new CustomException("执行流程逻辑 - 未找到对应的系统用户");
        }

        // 创建screen_uid_mapping对象并插入
        ScreenUidMapping mapping = new ScreenUidMapping();
        mapping.setRyUserId(sysUser.getUserId());
        mapping.setdUserId(deSysUser.getUserId());
        mapping.setEnabled(1);
        screenUidMappingMapper.insertScreenUidMapping(mapping);
    }

    @Transactional(rollbackFor = Exception.class)
    public void completeAccessApply(Map<String, Object> variables) {
        // 获取表单变量值
        String username = (String) variables.get("username2");
        String resourceId = (String) variables.get("screen2");
        SysUser sysUser = sysUserMapper.selectUserByUserName(username);
        ...
        if (screenAuth != null) {
            ...
            screenAuth.setAuthStatus(1);
            screenAuthMapper.updateScreenAuth(screenAuth);
        } else {
            // 如果记录不存在，新增一条记录
            screenAuthMapper.insertScreenAuth(ScreenAuth.builder()
                    .userId(userId)
                    .resourceId(resourceId)
                    .authStatus(1)
                    .build());
...
//        taskService.setOwner(flowTaskVo.getTaskId(), flowTaskVo.getAssignee());
    ...
    public AjaxResult myProcess(FlowQueryVo queryVo) {
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            flowTask.setDeleteReason(hisIns.getDeleteReason());
    ...
    public AjaxResult stopProcess(FlowTaskVo flowTaskVo) {
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(flowTaskVo.getInstanceId()).list();
        ...
        // 删除运行时流程
        runtimeService.deleteProcessInstance(flowTaskVo.getInstanceId(), "withdrawn");

        // 更新历史流程状态为已撤回
        historyService.createNativeHistoricProcessInstanceQuery()
            .sql("UPDATE act_hi_procinst SET DELETE_REASON_ = 'withdrawn' WHERE PROC_INST_ID_ = #{processInstanceId}")
            .parameter("processInstanceId", flowTaskVo.getInstanceId())
            .list();
