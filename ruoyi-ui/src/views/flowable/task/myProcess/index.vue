<template>
  <div class="app-container">
    ...
    <el-alert title="新增申请说明" type="success">
      <template slot='title'>
        <p>流程发起说明:</p>
        <div>1、点击新增申请。</div>
        <div>2、选择要申请的流程。</div>
    ...
    <el-table v-loading="loading" :data="myProcessList" border @selection-change="handleSelectionChange">
      ...
      <el-table-column label="流程类别" align="center" width="100px">
        <template slot-scope="scope">
          {{ scope.row.category || '常见流程' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="流程状态" align="center" width="100">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.deleteReason === 'withdrawn'" type="info" size="mini">已撤回</el-tag>
...
<!--          <label v-if="scope.row.candidate">{{scope.row.candidate}}</label>-->
      ...
      <el-table-column label="操作" width="150" fixed="right" class-name="small-padding fixed-width" align="center">
        <template slot-scope="scope">
          <el-button @click="handleFlowRecord(scope.row)" type="text" size="small">详情</el-button>
          <el-button v-if="!scope.row.finishTime" @click="handleStop(scope.row)" type="text" size="small">撤回</el-button>
          <el-button v-if="scope.row.finishTime" @click="handleDelete(scope.row)" type="text" size="small" v-hasPermi="['system:deployment:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
